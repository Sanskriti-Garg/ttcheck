...
function App() {
  ...
  const sendMessage = async () => {
    ...
    try {
      // Replace with your actual Swagger API endpoint
      const res = await fetch(`https://your-api.com/ask?query=${encodeURIComponent(input)}`);
      ...
      setMessages((prev) => [
        ...prev,
        { type: "bot", text: "⚠️ Failed to fetch response from API." },
      ]);
    }
